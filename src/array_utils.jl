## Indexing
Base.getindex(x::CasadiSymbolicObject, j::Union{Int,UnitRange{Int},Colon}) =
    get(x.sym, (1:length(x))[j] .- 1)

Base.getindex(
    x::CasadiSymbolicObject,
    j1::Union{Int,UnitRange{Int},Colon},
    j2::Union{Int,UnitRange{Int},Colon},
) = get(x.sym, LinearIndices(size(x))[j1, j2] .- 1)

Base.setindex!(x::CasadiSymbolicObject, v::Number, j::Union{Int,UnitRange{Int},Colon}) =
    set!(x.sym, (1:length(x))[if (j isa Int)
        j:j
    else
        j
    end] .- 1, v)
...
function Base.setindex!(
    ...
    set!(x.sym, LinearIndices(size(x))[J1, J2] .- 1, v)
end
